...
const batch=JSON.parse(readFileSync('scripts/sanity/step-7c-cloud.json'));
const before=JSON.parse(readFileSync('docs/seo-review/step-7c-evidence/cloud-before.json'));
const current=await c.getDocument(batch.id);
assert.deepEqual(current,before,'CMS changed since snapshot; stop and re-audit');
assert.equal(current._rev,batch.revision);assert.equal(current.slug.current,'cloud-devops');
assert.deepEqual(Object.keys(batch.set).sort(),['title','excerpt','contentHeading','content','process','workHeading','faqs','serviceArea'].sort());
...
if(apply){
 const after=await c.patch(batch.id).ifRevisionId(batch.revision).set(batch.set).commit({tag:'seo.step7c.cloud'});
 ...
 writeFileSync('docs/seo-review/step-7c-evidence/cloud-after.json',JSON.stringify(after,null,2)+'\n');
