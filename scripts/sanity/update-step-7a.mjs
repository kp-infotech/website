...
const batch=JSON.parse(readFileSync('scripts/sanity/step-7a-erp.json'));
const before=JSON.parse(readFileSync('docs/seo-review/step-7a-evidence/erp-before.json'));
const current=await c.getDocument(batch.id);
assert.deepEqual(current,before,'CMS changed since snapshot; stop and re-audit');
assert.equal(current._rev,batch.revision);assert.equal(current.slug.current,'erp-software');
assert.deepEqual(Object.keys(batch.set).sort(),['title','excerpt','contentHeading','content','process','faqHeading','faqs'].sort());
...
if(apply){
 const after=await c.patch(batch.id).ifRevisionId(batch.revision).set(batch.set).commit({tag:'seo.step7a.erp'});
 ...
 writeFileSync('docs/seo-review/step-7a-evidence/erp-after.json',JSON.stringify(after,null,2)+'\n');
