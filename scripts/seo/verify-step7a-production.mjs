...
const dir='docs/seo-review/step-7a-evidence/';const baseline=JSON.parse(fs.readFileSync('docs/seo-review/step-3-evidence/metadata-baseline.json'));const built=baseline.map(b=>inspect(fs.readFileSync('dist/client'+new URL(b.url).pathname+'index.html','utf8'),b.url));const pages=[];const raw=[];const errors=[];let cursor=0;
...
fs.writeFileSync('/private/tmp/kp-step7a-live-after.json',JSON.stringify(raw));
