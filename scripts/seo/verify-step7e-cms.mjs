...
const before=JSON.parse(readFileSync('/private/tmp/kp-step7e-cms-before.json')).filter(d=>['service','blogPost','caseStudy','industry','siteSettings','aboutPage'].includes(d._type));
const batch=JSON.parse(readFileSync('scripts/sanity/step-7e-ai.json'));const after=await c.getDocuments(before.map(d=>d._id));
...
const r={timestamp:new Date().toISOString(),documents:after.length,changedDocuments:1,errors:[]};writeFileSync('docs/seo-review/step-7e-evidence/cms-verification.json',JSON.stringify(r,null,2));console.log(r);
